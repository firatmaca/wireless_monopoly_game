...

{
    ...
    array[15].setval(390,0,"PORSUK GEZINTISI",200,50,0,0,0,0,0,false,"bank",0,"6");
    ...
    array[18].setval(600,0,"SU ISLERI",150,30,0,0,0,0,0,false,"bank",0,"7");
...
//    kamu[15].kamu("Çekilişten 50m Kazandın ","Bugün Doğum Günü Her Oyuncudan 25m Al","Tramvaya Kaçak Bindiğin İçin 100m öde"
//                  ," 6 Ganyandan 50m Kazandın ","Yıllık Faizlerden 100m Al","Vergi Kaçırdğın İçin Doğru Hapise Git",
//                  "Köpeğin Hastalandı Doktor Ücreti 50m Öde","Okul Taksidini Öde 50m",
//                  "Banka Hesabında Yanlışlık Olmuş 200m Al","Dedenden Miras Geldi 100m Al",
//                  "Sigoratadan Para geldi 100m Al","Güzellik Yarışmasında 3. Oldun 25 Al",
//                  "Araban Bozulmuş 200m Öde","Başlangıç Noktasına İlerle 200m Al ","Borsadan 50m Kazandın",
//                  "Vergi İadesi 25m Al");
...
void sec::on_pushButton_34_clicked()
{
    ...
    case 1:ui->z11->setVisible(true);
        ...
        ui->z66->setVisible(false);
        break;
    case 2:ui->z11->setVisible(false);
        ui->z22->setVisible(true);
        ui->z33->setVisible(false);
        ui->z44->setVisible(false);
        ui->z55->setVisible(false);
        ui->z66->setVisible(false);
        break;
    case 3:ui->z11->setVisible(false);
        ui->z22->setVisible(false);
        ui->z33->setVisible(true);
        ui->z44->setVisible(false);
        ui->z55->setVisible(false);
        ui->z66->setVisible(false);
        break;
    case 4:ui->z11->setVisible(false);
        ui->z22->setVisible(false);
        ui->z33->setVisible(false);
        ui->z44->setVisible(true);
        ui->z55->setVisible(false);
        ui->z66->setVisible(false);
        break;
    case 5:ui->z11->setVisible(false);
        ui->z22->setVisible(false);
        ui->z33->setVisible(false);
        ui->z44->setVisible(false);
        ui->z55->setVisible(true);
        ui->z66->setVisible(false);
        break;
    case 6:ui->z11->setVisible(false);
        ui->z22->setVisible(false);
        ui->z33->setVisible(false);
        ui->z44->setVisible(false);
        ui->z55->setVisible(false);
        ui->z66->setVisible(true);
        break;
    }
    int topz=zar1+zar2;
   qop=queue%nop;
...
for(int b=0; b<8 ; b++){
    ...
    {
     el++;
//  ui->messagebox->setText(QString::number(el));
    }
   }
if(el==7){
    for(int b=0 ; b<8;b++){
...
void sec::on_buildbutton_clicked()
{
    int counter=0,counter1=0,counter2=0;
    if(array[son[qop]].owner==players[qop].name){
        for(int i=0;i<40;i++){
            if(array[i].owner==players[qop].name && array[i].renk==array[son[qop]].renk){
                counter=counter+1;
            }else if(array[i].owner==players[qop].name && array[i].renk==array[son[qop]].renk && array[son[qop]].renk=="brown"){
                  counter1=counter1+1;
...
void sec::on_EXITBUTTON_clicked()
{
    this->close();
    MainWindow m;
    m.ex();
}
void sec::set(QByteArray Data){
    QString data=Data;
}
  void sec::getdat(QByteArray Data){
      QString dd=Data;
       qDebug()<<"GIRDI"<<dd;
       ui->label_4->setText(dd);
}
  */
